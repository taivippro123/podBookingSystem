...
function ManageRooms() {
  ...
  const handleCreateRoom = async () => {
    if (
      ...
      !newRoom.roomDetailsDescription ||
      newRoom.roomPricePerSlot < 0 ||
      newRoom.roomPricePerDay < 0 ||
      newRoom.roomPricePerWeek < 0
    ) {
      notification.error({
        message: "Error",
        description:
          "Please fill all fields and ensure price values are not negative.",
      });
      return;
    }
  ...
  const handleEditRoom = async () => {
    if (
      !roomToEdit.roomName ||
      !roomToEdit.roomType ||
      !roomToEdit.roomDescription ||
      !roomToEdit.roomDetailsDescription ||
      roomToEdit.roomPricePerSlot < 0 ||
      roomToEdit.roomPricePerDay < 0 ||
      roomToEdit.roomPricePerWeek < 0
    ) {
      notification.error({
        message: "Error",
        description:
    ...
    try {
      ...
      await axios.put(
        `http://localhost:5000/rooms/${roomToEdit.roomId}`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
  ...
  // Function to delete image by imageId
  const handleDeleteImage = async (imageUrl, index) => {
    try {
      // Call the API to delete the image from the database by imageUrl
      await axios.delete(
        `http://localhost:5000/room-image/${encodeURIComponent(imageUrl)}`
      );

      // Update the state to remove the image from the roomToEdit.images array
      const updatedImages = roomToEdit.images.filter((_, i) => i !== index);

      // Update roomToEdit state with the new images array
      setRoomToEdit((prevRoom) => ({
        ...prevRoom,
  ...
  return (
    <div>
      ...
      {isModalOpen && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            ...
            <button onClick={handleCreateRoom} className={styles.submitButton}>
              Create Room
            </button>
          </div>
        </div>
      ...
      {isEditModalOpen && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <span className={styles.closeIcon} onClick={closeEditModal}>
              ×
            </span>
            <h1>Edit Room</h1>
            ...
            >
              ...
              {temporaryImages.map((imageUrl, index) => (
                <div
                  key={`temp-${index}`}
                  style={{
                    display: "flex",
              ...
              {/* Display temporary image preview with delete button */}
              {tempImageUrl && (
                <div
                  style={{
                    display: "flex",
            ...
            <input
              ...
              onChange={(e) =>
                setRoomToEdit({ ...roomToEdit, roomName: e.target.value })
              }
              className={styles.inputField}
            />
            <input
              ...
              onChange={(e) =>
                setRoomToEdit({ ...roomToEdit, roomType: e.target.value })
              }
              className={styles.inputField}
            />
            <textarea
            ...
            <textarea
              ...
              onChange={(e) =>
                setRoomToEdit({
                  ...roomToEdit,
                  roomDetailsDescription: e.target.value,
                })
              }
              className={styles.textAreaField}
            ></textarea>
            ...
            <div className={styles.priceContainer}>
              <input
                ...
                onChange={(e) =>
                  setRoomToEdit({
                    ...roomToEdit,
                    roomPricePerSlot: e.target.value,
                  })
                }
                className={styles.inputField}
              />
            ...
            <div className={styles.priceContainer}>
              <input
                ...
                onChange={(e) =>
                  setRoomToEdit({
                    ...roomToEdit,
                    roomPricePerDay: e.target.value,
                  })
                }
                className={styles.inputField}
              />
            ...
            <div className={styles.priceContainer}>
              <input
                ...
                onChange={(e) =>
                  setRoomToEdit({
                    ...roomToEdit,
                    roomPricePerWeek: e.target.value,
                  })
                }
                className={styles.inputField}
              />
            ...
            <select
              ...
              onChange={(e) =>
                setRoomToEdit({ ...roomToEdit, roomStatus: e.target.value })
              }
              className={styles.selectField}
            >
              <option value="Available">Available</option>
            ...
            <button onClick={handleEditRoom} className={styles.submitButton}>
              Update Room
            </button>
          </div>
        </div>
